...
module internal ExcelAddressing = 
    ...
    ///Sheet!A1
    ///B3
    let parseExcelAddress workSheetName address =
        if address <> workSheetName then
            printfn $"address <> workSheetName: {address} <> {workSheetName}"
            ...
            let sheet = if sheetGroup.Success then sheetGroup.Value else workSheetName
            let row, column = parseCellAddress cellGroup.Value

            { Sheet = sheet; Row = row; Column = column }
        else 
            printfn $"address = workSheetName: {address} = {workSheetName}"
            { Sheet = workSheetName; Row = 0; Column = 0 }

    ///Parses an excel range of the form
    ///<ADDRESS>:<ADDRESS> | <ADDRESS>
    ///ADDRESS is parsed with the parseExcelAddress function
    let parseExcelRange workSheetName (range : string) =
        let addresses =
            range.Split(':')
            |> Array.map (parseExcelAddress workSheetName)
    ...
    let parseExcelRanges workSheetName (range : string) =
        range.Split(',')
        |> Array.map (parseExcelRange workSheetName)
        |> Array.toList
    ...
    let public getView (workbook : DataSet) sheetname range =
        ...
        let workSheetName = 
            let ss = 
                seq {
                    for s in worksheets do
                        yield s.TableName, s.Rows.Count
                }
                |> Seq.toArray
            if worksheets.Contains sheetname 
            then 
                printfn "Determine if %s existed: true, %A" sheetname ss
                sheetname
            else                
